            ...
            async Task RetryAsync(Func<Task> work, int attempts = 3, int delayMs = 200)
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            await work();  // Intenta ejecutar la operación
                        ...
                        catch (Exception)
                        {
                            ...
                            if (tries >= attempts)
                            {
                                throw;  // Si ya agotamos los reintentos, lanzamos el error
                            }
                            // Esperamos un poco antes de reintentar (backoff)
                            try { await Task.Delay(delayMs); } catch { }
                        }
                    }
                }

                // Configuramos Serilog (sistema de logging) con las opciones especificadas
                try
                {
                ...
                {
                    // Si falla la configuración avanzada, usamos un logger básico de consola
                    Console.WriteLine($"Failed to configure Serilog via SerilogConfigurator: {ex.Message}");
                    Log.Logger = new LoggerConfiguration().WriteTo.Console().CreateLogger();
                }
                
                // Creamos el logger que usaremos durante todo el proceso
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        // Verificamos si la BD ya existe y si tenemos permiso para borrarla
...
";
                            ...
                            if (!File.Exists(mdfPath))
                            {
                                try
                                {
                                    ...
                                    // Ejecutamos CREATE DATABASE especificando dónde guardar los archivos
                ...
                if (doSeed)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            Equipo GetOrCreateEquipo(string nombre, string descripcion)
                            {
                                var existing = equipoRepo.ReadFilter(nombre).FirstOrDefault();
                                if (existing == null)
                                {
                                    // Creamos nuevo equipo
                                    var equipo = equipoCEN.NewEquipo(nombre, descripcion);
                            ...
                            var eq1 = GetOrCreateEquipo("TeamAlpha", "Elite gaming team");
                            var eq2 = GetOrCreateEquipo("TeamBeta", "Competitive team");
                            ...
                            Console.WriteLine($"✓ CUSTOM: PromocionarAModerador executed");
                            ...
                            Console.WriteLine($"✓ CUSTOM: AprobarSiVotosUnanimes executed (result={aprobada})");
                            ...
                            Console.WriteLine($"✓ CP: CrearComunidadCP executed");
                            ...
                            Console.WriteLine($"✓ CP: UnirEquipoCP executed");
                            ...
                            if (inv1Reloaded != null)
                            {
                                ...
                                Console.WriteLine($"✓ CP: AceptarInvitacionCP executed");
                            }
                            else
                            {
                            ...
                            Console.WriteLine($"✓ CP: AprobarPropuestaTorneoCP executed (result={resultAprobacion})");
        ...
        {
            // Si ocurre un error no manejado, lo reportamos y devolvemos código de error 4
            Console.WriteLine($"InitializeDbService failed: {ex.Message}");
            return 4;  // Código de error 4: fallo general no esperado
        }
    }
}
