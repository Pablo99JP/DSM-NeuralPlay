...
namespace ApplicationCore.Domain.CEN
{
    public class ComunidadCEN
    {
        ...
        public IEnumerable<Comunidad> BuscarComunidadesPorNombreODescripcion(string filtro)
        {
            var repoObj = (object)_repo;
            var mi = repoObj.GetType().GetMethod("BuscarComunidadesPorNombreODescripcion");
            if (mi != null)
            {
                var res = mi.Invoke(repoObj, new object[] { filtro });
                if (res is IEnumerable<Comunidad> list) return list;
            }
